...
import { AlertCircle, Verified, OctagonX } from 'lucide-react';
import clsx from 'clsx';

export const GuardianInfo: React.FC<GuardianStepProps> = ({
  data,
  photos,
  errors,
  isContinuing,
  ...
  const guardianData = data.guardian1 || {
    full_name: '',
    contact: '',
    nin: '',
    email: '',
  ...
  const photoData = isContinuing ? photos.continuingGuardian1 : photos.guardian1;

  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    const { name, value } = e.target;
    
    if (name.startsWith('guardian1.')) {
      const field = name.replace('guardian1.', '') as keyof typeof guardianData;
      onUpdate({
        guardian1: {
          ...guardianData,
          [field]: value
  ...
  const handlePhotoChange = (file: File | null) => {
    if (file) {
      ...
      reader.onloadend = () => {
        if (isContinuing) {
          onPhotoUpdate('continuingGuardian1', file, reader.result as string, photoData.existingUrl);
        } else {
          onPhotoUpdate('guardian1', file, reader.result as string);
        }
      };
      reader.readAsDataURL(file);
    } else {
      if (isContinuing) {
        onPhotoUpdate('continuingGuardian1', null, '', photoData.existingUrl);
      } else {
        onPhotoUpdate('guardian1', null, '');
      }
    }
  };
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-lg font-medium">Primary Guardian Information</h2>
